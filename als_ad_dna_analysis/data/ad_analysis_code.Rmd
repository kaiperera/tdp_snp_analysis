...
```{r READ IN DATA}
ad_deepclip_data <- stream_in(file("C:/Users/Kai/Desktop/tdp_snp_analysis/als_ad_dna_analysis/data/ad_gwas1.json"))
```
...
```{r BINDING PROFILES}
ad_gwas1 <- jsonlite::fromJSON("C:/Users/Kai/Desktop/tdp_snp_analysis/als_ad_dna_analysis/data/ad_gwas1.json")$predictions
...
```{r READ IN BED}
bed_data <- read.table(
  unz(here::here("als_ad_dna_analysis/data/postar3_tardbp_reduced.bed.zip"),
...
ggplot(snps_in_binding_regions, aes(x = snp_in_tdp, y = min_diff)) +
  ...
  geom_hline(yintercept = CE_vline_min,
             size = 2,
             linetype = 'dotted')+
  labs(title = "Minimum Difference Distribution of SNPs based on Binding Region Presence",
       x = "SNP in Binding Region Status",
       y = "Minimum Difference") +
  theme_bw() +
  stat_compare_means(hjust = -0.8,
                     vjust = 15) 
...
ad_chi2 <- snps_in_binding_regions %>%
  ...
  dplyr::mutate(min_diff_binned = if_else(min_diff <=  -0.1398594 , "More Disruptive (<=CE_SNP)", "Less Disruptive (> CE_SNP)"))
